...
editor to follow lab completion [[https://notes.inria.fr/tFihp6A5TA-fOaUIvqNdyw][here]].
...
(let* ((base-dir "~/Documents/site/teachings/2021-2022/")
       (export-dir (concat base-dir "os-polytech/")))
...
sudo snap install openstackclients --channel=ussuri/stable
...
the dashboard (see the [[https://docs.openstack.org/liberty/install-guide-obs/keystone-openrc.html][client environment documentation]]).
...
3. +Suspend it, note the footprint on the ram of the hypervisor, and
   resume it;+ Does not work right now, do not try it if you do not want to have to re-create it 😒
...
* Automatize the Deployment with Heat :noexport:
:PROPERTIES:
:CUSTOM_ID: sec:heat
:END:
...
* Deploy a WordPress as a Service (as a Heat DevOps) :noexport:
:PROPERTIES:
