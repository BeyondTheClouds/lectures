...
#+BEGIN_SRC bash
curl https://github.com/BeyondTheClouds/lectures/blob/da421e2/2020-2021/os-imt/tp.tar.gz?raw=true -o tp.tar.gz -L
tar xzf tp.tar.gz
cd lab-os
#+END_SRC
...
security group rules of the ~admin~ project.
...
network is allowed for incoming (~ingress~) or outgoing (~egress~)
traffic"/.
#+BEGIN_src bash
...
#+LINK: cdn-url      https://github.com/BeyondTheClouds/lectures/blob/da421e2/%s?raw=true
