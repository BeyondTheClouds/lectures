...
editor to follow lab completion [[https://pad.inria.fr/p/np_HcC3U1VsFnNgT1af_Nuage-2020-2021][here.]]
...
(let* ((output-dir "output")
  ...
  ;; Make the tp.tar.gz
  (shell-command (format "tar czf tp.tar.gz --exclude=index.html --transform 's|^%s|lab-os|' %s"
...
https://pad.inria.fr/p/np_HcC3U1VsFnNgT1af_Nuage-2020-2021.
...
7. Fill in the password of Step 4 to the question ~Enter Private Key
   Password:~.
...
#+BEGIN_SRC bash
curl https://github.com/BeyondTheClouds/lectures/blob/3bdbd4e/2020-2021/os-imt/tp.tar.gz?raw=true -o tp.tar.gz -L
tar xzf tp.tar.gz
cd lab-os
#+END_SRC
...
#+BEGIN_SRC bash
openstack server list --os-auth-url=http://<ip-of-your-lab-machine>:5000/v3/ \
                        --os-username=admin \
                        --os-password=lab-os \
...
*** COMMENT Make the VM reaches the world
...
launch [[https://en.wikipedia.org/wiki/MariaDB][MariaDB]] on a cirros.
...
# *Note for the teacher:* The [[lst:undo-kvm]] code undoes the setting of
# kvm, so students first come with qemu and then set kvm to first feel
# the difference of speed and second see how to configure nova.  This
# bash snippet is tangle into [[file:setup.sh]] and could be disable to
# manage time.  Simply comment that headline to disable it.
#+NAME: lst:undo-kvm
#+BEGIN_SRC bash :exports none
...
processes the REST request.  The API, in turn, calls the
...
its ~nova-compute~.  Finally, the ~nova-compute~ asks to the
underlying hypervisor to start the VM.

In your current setup, the hypervisor of your ~nova-compute~ runs
[[https://en.wikipedia.org/wiki/QEMU][QEMU]].  QEMU is a free emulator for hardware virtualization.  It
supports a large variety of guest operating systems, but the emulation
is a bit slow.  Fortunately, QEMU can be used with [[http://www.linux-kvm.org/][KVM]] to run virtual
machines at near-native speed.  KVM (Kernel-based Virtual Machine) is
a free full virtualization solution for Linux that takes advantage of
x86 hardware extensions (Intel VT or AMD-V).
...
#+BEGIN_note
You can check that the new VM effectively uses KVM by looking at its
configuration file in the hypervisor.
...
If the domain is ~kvm~ instead of ~qemu~ then the reconfiguration of
nova has been well taken into account.
#+END_note
...
how to deploy applications on OpenStack.  Templates in the following
are available under the ~rsc/heat-templates/~ directory.  You may also
find interesting examples in the [[os-doc:heat,template_guide/basic_resources.html][Heat documentation]], or on the
[[https://github.com/openstack/heat-templates][heat-templates repository]].
...
each of you (not admin).  As a preamble, you should connect to the
Grid'5000 VPN (Sec.\nbsp{}[[#sec:assign-lab]]) and then do the following.  You
can do it on your own machine, or from the rennes frontend (~ssh
access.grid5000.fr; ssh rennes~).

- [[https://github.com/openstack/python-openstackclient/tree/stable/rocky#getting-started][Install the OpenStack CLI]] and [[https://github.com/openstack/python-heatclient/tree/stable/rocky][Heat CLI]].  Here is the process for
  Grid'5000.  You may omit the 3 first lines if you do it on your own
  machine.
...
on declaring outputs, so that a stack can return a set of attributes
(e.g., the IP address of a deployed VM).
...
The source code of Heat also list [[https://github.com/openstack/heat/blob/0703ca7bb19ca3bb06009c828a66bababf9970b8/heat/engine/resources/openstack/nova/server.py#L646-L736][extra attributes]] that let you find
the IP address such as ~first_address~, but that one is deprecated
though.
: get_attr: [heat-vm, first_address]

The [[horizon-url][horizon dashbord]] has an "Orchestration" tab with a good list of
...
- ~wp-vm.yaml~  :: Contains the description of the VM running the Web
  server and serving WordPress.
- ~wp-app.yaml~ :: Contains the description of the WordPress
  application (glues the ~db-vm.yaml~ and ~web-vm.yaml~ together).

Once it is deployed, you should be able to reach the WordPress service by
going on [[http://<wp-vm-fip-address>/wp]].

#+BEGIN_comment
# solution
Find the solution in the ~rsc/heat-templates/wordpress/~ directory of
the tarball and in the following subsections.  Create the WordPress
application and output its endpoint with the next commands.

#+BEGIN_src bash
openstack stack create -t ./rsc/heat-templates/wordpress/wp-app.yaml wp1
echo "http://$(openstack stack output show wp1 public-ip -c output_value -f value)/wp"
#+END_src
#+END_comment
# solution
#+END_do

** COMMENT Database VM template                                    :solution:
...
** COMMENT Web VM template                                         :solution:
...
** COMMENT Wordpress application template                          :solution:
...
* Appendix
** COMMENT Vagrantfile
[[https://en.wikipedia.org/wiki/Vagrant_%28software%29][Vagrant]] is an open-source software to build development environment
using a declarative approach.  In addition to Vagrant, we recommend to
install the [[https://github.com/vagrant-libvirt/vagrant-libvirt#installation][Vagrant Libvirt Provider]] in order to take advantage of the
hardware virtualization.

Copy the following content into a file named "Vagrantfile".  Review
the content, especially the number of CPU, memory size and the
private/public network.
#+include: "Vagrantfile" src ruby
...
the content of the ~/vagrant~ directory in the lab machine with the
~vagrant rsync~ command.
...
on a loop with ~mpv --vo caca --no-audio --loop vid.mp4~ after SSH on it.
#+begin_src bash
...
#+begin_src bash :tangle output/rsc/live-migrate-do-it.sh
#!/usr/bin/env bash
...
** COMMENT Misc
Pad pour la prise de note commune
https://pad.inria.fr/p/np_HcC3U1VsFnNgT1af_Nuage-2020-2021

#+begin_src bash
sudo useradd -g 100 -G adm,sudo -m -s /bin/bash -c 'tp-os ronan-alexandre.cherrueau@inria.fr' tp-os
sudo passwd tp-os


#+end_src

Tunneling from the outside world (my machine) to my lab machine
throughout the frontend
: ssh -NL 8080:<ip-lab-machine>:80 <frontend>

Now I can access the service of ~http://ip-lab-machine:80~ on my
machine from the URL https://localhost:8080

: ssh -NL 8080:parapide-7.rennes:80 rennes.g5k

: ssh -NL 8080:193.52.86.XXX:80 nom_connexion@172.19.64.240 -p 20XXX

: ssh -NL 8080:193.52.86.102:80 ronan@ttyd.user-content.tk

Share a terminal
: tmux -S /tmp/ronan-alebre new
: tmux -S /tmp/ronan-alebre attach
...
sudo extcidr=10.20.20.0/24  # find it with `sudo iptables -t nat -L`
...
# test mysql, mariadb, memcached
apt list --installed | grep -E 'mariadb|memcached'
sudo apt remove --yes --quiet --autoremove mariadb-common mariadb-server memcached

# To create account on frontend
echo "Enter password for students: "
read PASSWD
while read line ; do login=$(echo $line |awk -F"," '{print $1}') ;  mail=$(echo $line|awk -F"," '{print $2}') ;  sudo useradd -g 100 -m -s /bin/bash -c "$mail" $login ;  done </tmp/studentlist
while read line ; do login=$(echo $line |awk -F"," '{print $1}') ;  echo $login:$PASSWD |sudo chpasswd ; done </tmp/studentlist
# if needed
while read line ; do login=$(echo $line |awk -F"," '{print $1}') ;  sudo deluser $login ; done </tmp/studentlist

: sudo useradd -g 100 -m -s /bin/bash -c "xingshuo.cha@etd-univ-nantes.fr" xcha
: echo "xcha:os-polytech" | sudo chpasswd

while read line ; do login=$(echo $line |awk -F"," '{print $1}') ;  echo $login:$PASSWD |sudo chpasswd
...
#+LINK: cdn-url      https://github.com/BeyondTheClouds/lectures/blob/3bdbd4e/%s?raw=true
