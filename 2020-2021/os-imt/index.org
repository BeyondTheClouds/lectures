...
- Reimport (or maybe recreate) your admin SSH key
  (Sec.\nbsp{}[[#sec:enc-trust]]).
- Reimport the source of this lab (Sec. \nbsp{}[[#sec:rscs-lab]]).
...
outputs:
  public-ip:
    description: IP address of the created instance running MariaDB
    value: { get_attr: [wordpress, networks, public, 0] }
#+END_src

#+BEGIN_src bash
openstack stack create -t ./rsc/heat-templates/wordpress/wp-app.yaml wp1
FIP_RSC_ATTRIBUTES=$(openstack stack resource show -c attributes -f value wp1 public-ip)
python -c "print('http://%s/wp' % ${FIP_RSC_ATTRIBUTES}['floating_ip_address'])"
#+END_src
