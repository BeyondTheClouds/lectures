...
1. Any intercommunication among hosts of the same project :: This is
   the first line.  It should be read as "/Incoming traffic
   (~ingress~) between/ /hosts of ~192.162.222.*~ of any protocol
   (~None~) are allowed/".
2. Any kind of outgoing communications :: This is the second line.  It
   should be read as "/Outgoing traffic (~egress~) to anywhere (~0.0.0.0/0~)
   and of any protocol (~None~) are allowed/".
