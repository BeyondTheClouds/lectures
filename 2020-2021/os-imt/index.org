...
# openstack server list --os-auth-url=http://$(ip addr show eno1 | head -n3 | tail -n1 | awk '{print $2}' | cut -f1 -d'/'):5000/v3/ \
#                         --os-username=admin \
#                         --os-password=lab-os \
...
By default, OpenStack allows any intercommunication among hosts of the
same project. This is the first line. It should be read as "/Incoming
communication (~egress~) between host of ~192.162.222.*~ of any
protocol (~None~) is allowed/)".  OpenStack also allows any kind of
outgoing requests. This the second line that should be read as
"/Outgoing communication (~egress~) to anywhere (~0.0.0.0/0~) and of
any protocol (~None~) is allowed.  And that's it.  Since there are no
more rules, it means that OpenStack prevents all other ingress
communications including communications on ~10.20.20.*~.
#+END_solution
