...
editor to follow lab completion [[https://pad.inria.fr/p/np_KeAkcZTawSB7hPq3_os-poly-grp2][here]].
...
- [[#automatize-the-deployment-with-heat][Automatize the Deployment with Heat]]
  - [[#install-the-heat-orchestration-service-on-microstack][Install the Heat orchestration service on microstack]]
  - [[#boot-a-vm][Boot a VM]]
...
(let* ((base-dir "~/prog/rcherrueau.github.com/teaching/")
       (export-dir (concat base-dir "2020-2021/os-polytech/")))
...
installed -- use ~lsb_release -d~ to check it out -- you should start
a proper VM using, for instance, the [[*Vagrantfile][Vagrantfile]] given in the
...
Lab machines aren't available publicly unfortunately.  They are hide
behind Polytech private network.  Fortunately, a frontend is available
from the outside world at the address ~163.172.58.84~ that let us
reaches Lab machines.  SSH on that frontend using the login and
password given in the pad of the collaborative editor and then, SSH on
your Lab machine.  Password for ~invité~ on the lab machine is ~d012~.
...
:END:
Get the resources of the lab at [[cdn-url:2020-2021/os-polytech/tp.tar.gz]].

#+BEGIN_SRC bash
curl https://github.com/BeyondTheClouds/lectures/blob/16dda68/2020-2021/os-polytech/tp.tar.gz?raw=true -o tp.tar.gz -L
tar xzf tp.tar.gz
cd lab-os
#+END_SRC
...
#+BEGIN_solution
#+BEGIN_SRC bash
openstack server create --wait --image debina-10 \
...
** COMMENT Run VMs at (near-)native speed
...
** Install the Heat orchestration service on microstack
The documentation to install Heat on Ubuntu is available [[os-doc:heat,install/install-ubuntu.html][online]]. The
...
#+BEGIN_src bash :tangle output/install-heat.sh :shebang #!/usr/bin/env bash
set -o errexit
set -o xtrace

MYSQL_PASS=$(snap get microstack config.credentials.mysql-root-password)
RMQ_PASS=$(snap get microstack config.credentials.rabbitmq-password)

# Create the Heat database
cat << EOSQL | sudo microstack.mysql --user=root --password="${MYSQL_PASS}"
...
crudini --set /etc/heat/heat.conf DEFAULT transport_url "rabbit://openstack:${RMQ_PASS}@10.20.20.1"
...
openstack server add floating ip user \
  $(openstack floating ip create -c floating_ip_address -f value external)
openstack server ssh --login debian --identity ./admin.pem --address-type external user
...
https://pad.inria.fr/p/np_e8il1vJgfq4KIf9G
...
#+LINK: cdn-url      https://github.com/BeyondTheClouds/lectures/blob/16dda68/%s?raw=true
