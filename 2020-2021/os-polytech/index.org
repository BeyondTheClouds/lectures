...
editor to follow lab completion [[https://pad.inria.fr/p/np_NuI6nLexB8GRMIBL_os-poly-grp1][here]].
...
with, at least, 8 GiB of RAM and 2 CPU.  We are going to use Dell R320
from room D011.  If you have a *different version* of Ubuntu installed
-- use ~lsb_release -d~ to check it out -- you should start a proper
VM using, for instance, the [[*Vagrantfile][Vagrantfile]] given in the Appendix.
Alternatively, you can also use your own machine.
...
:END:
Here is the assignation list of team per Lab machines:
- ~193.52.86.102~ ::
- ~193.52.86.115~ ::
- ~193.52.86.122~ ::
- ~193.52.86.126~ ::
...
behind polytech private network.  You have to be in the VPN of the
Université de Nantes and SSH on your machine thanks to a -- sort of --
router between the network of the Université de Nantes and Lab
machines: ~172.19.64.240~.  That router is configured with DNAT to map
a specific port to each Lab machine so you can SSH on it.  Here are
the SSH commands for the corresponding machine.  Password is ~d012~.
- ~ssh -l invité -p 20102 172.19.64.240~ ↦ ~193.52.86.102~
- ~ssh -l invité -p 20115 172.19.64.240~ ↦ ~193.52.86.115~
- ~ssh -l invité -p 20122 172.19.64.240~ ↦ ~193.52.86.122~
- ~ssh -l invité -p 20126 172.19.64.240~ ↦ ~193.52.86.126~
- ~ssh -l invité -p 20133 172.19.64.240~ ↦ ~193.52.86.133~
- ~ssh -l invité -p 20147 172.19.64.240~ ↦ ~193.52.86.147~
- ~ssh -l invité -p 20203 172.19.64.240~ ↦ ~193.52.86.203~
...
#+BEGIN_SRC bash
curl https://github.com/BeyondTheClouds/lectures/blob/b70819d/2020-2021/os-polytech/tp.tar.gz?raw=true -o tp.tar.gz -L
tar xzf tp.tar.gz
cd lab-os
#+END_SRC
...
3. Suspend it, note the footprint on the ram of the hypervisor, and resume it;
...
#+LINK: cdn-url      https://github.com/BeyondTheClouds/lectures/blob/b70819d/%s?raw=true
