...
See also, for examples of security groups
https://docs.openstack.org/neutron/latest/admin/deploy-lb-selfservice.html#verify-network-operation.
...
* COMMENT Deploy a Wordpress as a Service
...
database (MySQL). Apache serves the PHP code of WordPress and stores
...
make a bash script that automatically starts :

- ~wordpress-db~   :: A VM that contains the MySQL database for
     Wordpress.
- ~wordpress-app~  :: A VM that contains a web server and serves the
     Wordpress CMS.

The ~rsc~ directory provides bash scripts to deploy the MySQL database
and web server of WordPress. Review it before going further. Also,
remind to [[#sec:debian9-ftw][clean your environment]].
#+END_do
...
with the ~rsc/install-mariadb.sh~ script thanks to the ~--user-data
...
flavor, ~test~ network and ~admin~ key-pair. Also, provision your VM
with the ~rsc/install-wp.sh~ script thanks to the ~--user-data
