...
** Installation on microstack                                      :noexport:
See https://docs.openstack.org/heat/train/install/install-ubuntu.html
...
microstack.openstack endpoint create --region microstack \
  orchestration public http://10.20.20.1:8004/v1/
microstack.openstack endpoint create --region microstack \
  orchestration internal http://10.20.20.1:8004/v1/
microstack.openstack endpoint create --region microstack \
  orchestration admin http://10.20.20.1:8004/v1/
  # http://controller:8004/v1/%\(tenant_id\)s
microstack.openstack endpoint create --region microstack \
  cloudformation public http://10.20.20.1:8000/v1
microstack.openstack endpoint create --region microstack \
  cloudformation internal http://10.20.20.1:8000/v1
microstack.openstack endpoint create --region microstack \
  cloudformation admin http://10.20.20.1:8000/v1

# Create the heat domain
microstack.openstack domain create --description "Stack projects and users" heat
...
microstack.openstack role add --project admin --user demo heat_stack_owner
microstack.openstack role create heat_stack_user

# Install OpenStack heat
apt update
apt install heat-api heat-api-cfn heat-engine crudini
