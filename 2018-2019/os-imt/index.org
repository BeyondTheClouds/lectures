...
#+LINK: cdn-url   https://github.com/BeyondTheClouds/lectures/blob/34fc596c4b8404e60d6f34f4223dbe4703c40c3b/%s?raw=true
...
Find the slides of the lecture [[cdn-url:2018/os-polytech/docs/CloudFogEdgeIntro.pdf][here]] and [[cdn-url:2018/os-polytech/docs/openstack-slides.pdf][there]]. This document is an [[https://orgmode.org/][Org
mode]] document, you can find its source [[base-url:index.org][here]].
#+END_abstract
...
publicly available ~access.grid5000.fr~ machine, and from there, do a
second SSH connection to the Lab machine. But this solution is fairly
limited since it doesn't give an access to services from your own
machine[fn:g5k-tunnel].
...
#+BEGIN_solution
Find the solution in the [[cdn-url:2019/os-imt/rsc/wordpress-deploy.sh][~rsc/wordpress-deploy.sh~]] script.
#+END_solution
#+END_do
...
VM with the [[cdn-url:2019/os-imt/rsc/install-mariadb.sh][~rsc/install-mariadb.sh~]] script thanks to the ~--user-data
...
flavor, ~test~ network and ~admin~ key-pair. Also, contextualize your
VM with the [[cdn-url:2019/os-imt/rsc/install-wp.sh][~rsc/install-wp.sh~]] script thanks to the ~--user-data
