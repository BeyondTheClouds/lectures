...
- index.org :: The current subject in text format.
...
- glance-* :: Glance to manage VM images.
...
- neutron-* :: Neutron to manage networks
- nova-* :: Nova to manage VM
...
#+BEGIN_SRC bash :tangle ./teardown.sh :shebang #!/usr/bin/env bash :exports none
...
Unfortunately this feature is disabled with Snap microstack but as
every devops, you will prefer to access to your vm by the command line
interface as desribed in the next section.
...
You are luck, OpenStack provides such a command line interface. 
...
through the horizon interface by clicking on the top right corner on
the admin dropdown list and get the RC V3 file.

To setup your environment please source this file.
: vagrant@openstack:~$ source ~/Downloads/admin-openrc
...
vagrant@openstack:~$ openstack server add floating ip\
  cli-vm\
  $(openstack floating ip create external -c floating_ip_address -f value)
#+END_EXAMPLE
...
to boot. 
#+BEGIN_do
Ping it on its floating IP
: vagrant@openstack:~$ ping <floating-ip> # floating-ip is 192.168.143.*
Does it work? Why?  OpenStack is using different security rules, find
which rules should be set in order to allow icmp packets as well as
ssh connections to the VM.
#+END_do

#+BEGIN_COMMENT
#+BEGIN_SRC bash
SECGROUP_ID=`microstack.openstack security group list --project admin -f value -c ID`
openstack security group rule create $SECGROUP_ID --proto tcp --remote-ip 0.0.0.0/0 --dst-port 22
...
Go on and play with the
~openstack~ cli. For instance, list all features offered
...
3. Create a snapshot of ~cli-vm~ (for this question, you should play directly wit the nova CLI)
4. Boot a new machine ~cli-vm-clone~ from the snapshot.
5. Delete ~cli-vm-clone~;

#+BEGIN_COMMENT
#+BEGIN_SRC bash
sudo snap install openstackclients --edge --classic
openstack server ssh cli-vm -l cirros
openstack server suspend cli-vm
nova image-create cli-vm cli-vm-img
openstack image list
openstack server create --flavor m1.tiny --network test --image cli-vm-img cli-vm-clone
#+END_SRC
#+END_COMMENT
...
address of the MySQL server to this script before running it.
