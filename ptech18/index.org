...
#+LINK: horizon-url  http://localhost:8080/%s
...
through the Horizon interface by clicking on the top right corner of
the admin dropdown list and get the RC V3 file (or by clicking on that
[[horizon-url:project/api_access/openrc/][link]]).
...
with ~openstack console log show cli-vm~. The VM finished to boot when
last lines are:
#+BEGIN_EXAMPLE
...
ping your VM from an external network (e.g., the host machine). You
have to manually affect a floating IP of the ~external~ network to
your machine if you want it to be pingable from the host.
#+BEGIN_SRC bash
...
3. Create a snapshot of ~cli-vm~ (Hint: use the ~nova~ CLI directly)
...
# 3.
nova image-create cli-vm cli-vm-img; openstack image list
# 4.
